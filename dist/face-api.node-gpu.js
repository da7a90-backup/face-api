...
__reExport(tfjs_esm_exports, tfjs_node_gpu_star);
import * as tfjs_node_gpu_star from "@tensorflow/tfjs-node-gpu";
var version = "4.9.0";
var version2 = "4.9.0";
var version3 = "4.9.0";
var version4 = "4.9.0";
...
async function extractFaceTensors(imageTensor, detections) {
  ...
  return tfjs_esm_exports.tidy(() => {
    ...
    const faceTensors = boxes.filter((box) => box.width > 0 && box.height > 0).map(({ x, y, width, height }) => tfjs_esm_exports.slice3d(imageTensor.as3D(imgHeight, imgWidth, numChannels), [y, x, 0], [height, width, numChannels]));
    return faceTensors;
  });
}
...
function denseBlock3(x, denseBlockParams, isFirstLayer = false) {
  return tfjs_esm_exports.tidy(() => {
    const out1 = tfjs_esm_exports.relu(
      isFirstLayer ? tfjs_esm_exports.add(
        tfjs_esm_exports.conv2d(x, denseBlockParams.conv0.filters, [2, 2], "same"),
...
function denseBlock4(x, denseBlockParams, isFirstLayer = false, isScaleDown = true) {
  return tfjs_esm_exports.tidy(() => {
    const out1 = tfjs_esm_exports.relu(
      isFirstLayer ? tfjs_esm_exports.add(
        tfjs_esm_exports.conv2d(x, denseBlockParams.conv0.filters, isScaleDown ? [2, 2] : [1, 1], "same"),
        denseBlockParams.conv0.bias
...
function convLayer(x, params, padding = "same", withRelu = false) {
  return tfjs_esm_exports.tidy(() => {
    const out = tfjs_esm_exports.add(
      tfjs_esm_exports.conv2d(x, params.filters, [1, 1], padding),
      params.bias
    );
...
function extractConvParamsFactory(extractWeights, paramMappings) {
  return (channelsIn, channelsOut, filterSize, mappedPrefix) => {
    const filters = tfjs_esm_exports.tensor4d(
...
function fullyConnectedLayer(x, params) {
  return tfjs_esm_exports.tidy(() => tfjs_esm_exports.add(
    tfjs_esm_exports.matMul(x, params.weights),
    params.bias
  ));
}
...
var FaceLandmark68NetBase = class extends FaceProcessor {
  postProcess(output, inputSize, originalDimensions) {
    ...
    return tfjs_esm_exports.tidy(() => {
      ...
      const landmarkTensors = output.mul(tfjs_esm_exports.fill([batchSize, 136], inputSize, "float32")).sub(tfjs_esm_exports.stack(Array.from(Array(batchSize), (_, batchIdx) => createInterleavedTensor(
        getPaddingX(batchIdx),
        getPaddingY(batchIdx)
      )))).div(tfjs_esm_exports.stack(Array.from(Array(batchSize), (_, batchIdx) => createInterleavedTensor(
  ...
  async detectLandmarks(input) {
    const netInput = await toNetInput(input);
    const landmarkTensors = tfjs_esm_exports.tidy(
      () => tfjs_esm_exports.unstack(this.forwardInput(netInput))
    );
...
function extractorsFactory3(extractWeights, paramMappings) {
  function extractFilterValues(numFilterValues, numFilters, filterSize) {
    ...
    return tfjs_esm_exports.tidy(
      () => tfjs_esm_exports.transpose(
        tfjs_esm_exports.tensor4d(weights, [numFilters, depth, filterSize, filterSize]),
        [2, 3, 1, 0]
      )
    );
  }
...
function extractParams5(weights) {
  ...
  const fc = tfjs_esm_exports.tidy(
    () => tfjs_esm_exports.transpose(tfjs_esm_exports.tensor2d(extractWeights(256 * 128), [128, 256]), [1, 0])
  );
...
function extractParams6(weights) {
  ...
  const extra_dim = tfjs_esm_exports.tensor3d(
...
function depthwiseConvLayer(x, params, strides) {
  return tfjs_esm_exports.tidy(() => {
    ...
    out = tfjs_esm_exports.batchNorm(
      out,
...
function decodeBoxesLayer(x0, x1) {
  ...
  return tfjs_esm_exports.transpose(
    tfjs_esm_exports.stack([
...
function outputLayer(boxPredictions, classPredictions, params) {
  return tfjs_esm_exports.tidy(() => {
    ...
    let boxes = decodeBoxesLayer(
      tfjs_esm_exports.reshape(tfjs_esm_exports.tile(params.extra_dim, [batchSize, 1, 1]), [-1, 4]),
      tfjs_esm_exports.reshape(boxPredictions, [-1, 4])
    );
...
function boxPredictionLayer(x, params) {
  return tfjs_esm_exports.tidy(() => {
    ...
    const boxPredictionEncoding = tfjs_esm_exports.reshape(
      convLayer(x, params.box_encoding_predictor),
      [batchSize, -1, 1, 4]
    );
    const classPrediction = tfjs_esm_exports.reshape(
